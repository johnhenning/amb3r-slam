...
def reference_order(n, reference):
...
class DA3Model:
    ...
    def __init__(self, checkpoint="depth-anything/DA3-SMALL", device="cuda", resolution=504):
...
class TorchModel:
    def __init__(self, checkpoint, device="cpu", size=None):
        import torch
        ...
        self.size = tuple(size or state["image_size"])
...
class ONNXModel:
    ...
    def reconstruct(self, frames, reference=0):
        ...
        data = dict(zip(self.metadata["outputs"], values))
        return Reconstruction(
            *[
