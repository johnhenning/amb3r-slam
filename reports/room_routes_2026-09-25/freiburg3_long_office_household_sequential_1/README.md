...
|---|---:|---:|---:|---:|---:|---:|---:|
| rgbd_dataset_freiburg3_long_office_household | 259 | 258 | 0.4118 | 0.1505 | 0.0408 | 2.076 | 0.301 |
...
Sampled 259 of 2585 RGB frames over 87.00 s; 99.6% matched ground truth. Backend: 63 submaps, 205 edges. Push p50/p95: 804.6/15459.9 ms.

Execution: sequential; pending-window limit: 2; peak outstanding: 0; submission backpressure: 0.0 ms.

![Trajectory, absolute error and latency](rgbd_dataset_freiburg3_long_office_household/diagnostics.png)

[Metrics and configuration](rgbd_dataset_freiburg3_long_office_household/results.json) · [Corrected trajectory](rgbd_dataset_freiburg3_long_office_household/trajectory.tum) · [Online trajectory](rgbd_dataset_freiburg3_long_office_household/trajectory_online.tum) · [Latency samples](rgbd_dataset_freiburg3_long_office_household/latency.csv)

## Interpretation and limits

This first real-data baseline exercises pretrained inference and the full Python SLAM path on one sequence from one standard dataset. Sampling and a small CPU model make it a reduced benchmark, not a full-rate TUM evaluation. The paper uses a stronger backend; these results cannot be compared directly with its tables. Ground-truth alignment uses the complete run and is offline. Loops are enabled but successful loop closure is not guaranteed. No held-out-training-data claim is made for the upstream pretrained checkpoint.
